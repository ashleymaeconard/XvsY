...
rule make_boxplots:
	...
	shell:
		"python scripts/allBoxPlots.py -f {input} -d /data/compbio/inathoo/snakemake_test/test7/data -s {output}"	
...
rule make_XAboxplots:
	...
	shell:
		"python scripts/xaBoxPlotsfinal.py -f {input} -d /data/compbio/inathoo/snakemake_test/test7/data -s {output}"	
...
rule global_boxplots:
	...
	shell:
		"python scripts/global_XAv5.py -f {input} -s {output} -g /data/compbio/inathoo/snakemake_test/test7/scripts"
