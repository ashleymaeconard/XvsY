...
if (length(args)==0) {
      stop("Pass in 11 arguments. Type:
       Rscript DESeq2.r 
            ...
            10) SIGNIFICANCE TEST (wald, lrt)
            11) GENE READ COUNT FILTERING THRESHOLD (eg. 3)")
...
ORGANISM <- args[8]
CONTROL <- args[9]
SIGNIFICANCE_TEST <- args[10]
FILTERING_THRESHOLD <- as.numeric(args[11])
...
main <- function(){
    ...
    ALL_DEG_DIR <- paste(OUTPUTDIR,"other_deseq2_outputs",sep="/") #for all gene sets
    FULL_OUTDIR <- paste(ALL_DEG_DIR,paste(CONDITION,"results",sep="_"),sep="/")
    DATA_DIR <- paste(OUTPUTDIR,"data",sep="/") #for up and down reg sets
    if (!dir.exists(ALL_DEG_DIR)){
        dir.create(ALL_DEG_DIR)
    } else {
        print("other deseq2 outputs directory already exists.")
    }
    ...
    casecontrolkeep <- c(CONDITION, CONTROL)
    metaData <- metaData[metaData$condition %in% casecontrolkeep,]
    col_for_index="ID"
    # Format metadata
    ...
    # Filter count matrix based on number of read counts threshold
    # casecols <- grep(CONDITION, colnames(countData))
    # print(casecols)
    # casecontrolkeep <- c(CONDITION)
    metaDataConditon <- metaData[metaData$condition %in% c(CONDITION),]
    # print(metaDataConditon)
    casecols <- rownames(metaDataConditon)
    # print(casecols)
    summed <- rowSums(countData[, casecols])/(length(casecols))
    # print(head(summed,20))
    # print(FILTERING_THRESHOLD)
    genes_to_keep <- which(summed > FILTERING_THRESHOLD)
    # print(head(genes_to_keep,20))
    # print(head(countData,20))
    countData <- countData[genes_to_keep,]
    # print(head(countData,20))
    print(head(countData))
...
#     dev.off()
    ...
    matW <- betasTCW[topGenesW]
    matLRT <- betasTCLRT[topGenesLRT]
    matNLRT <- betasTCLRT[topGenesNLRT]
