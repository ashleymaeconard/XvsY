#!/bin/bash
#Script to find intersections between FOUR or SIX sets of genes and visualize them in venn diagrams and bar plots
#based on the FlyBase ID of these genes, and generates BED files each of these unique intersections.
#For genes sets SEPARATED into up and downregulated.
if [ $# -ne 1 ]; then
	echo "Usage: ./intersections_separated_regulated_GeneSets.sh
...
if [[ $COUNTER -eq 6 ]]
then
intervene venn -i ${DIR}/*.txt --type list --colors=green,palegreen,darkorange,navajowhite,purple,plum --figtype png --fontsize 11 --save-overlaps -o "$DIR/vennDiagram"
else
intervene venn -i ${DIR}/*.txt --type list --colors=green,palegreen,purple,plum --figtype png --fontsize 11 --save-overlaps -o "$DIR/vennDiagram"
fi
